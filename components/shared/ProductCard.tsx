...
export default function ProductCard({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative bg-white rounded-[12px] border border-gray-100 shadow-sm hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col",
        className
      )}
    >
      {/* Badge */}
      <div className="absolute top-4 left-4 z-10">
        <span className={cn("text-xs font-bold px-3 py-1 rounded-full", badgeClass)}>
          {badge}
        </span>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
          ...
          <Link
            href={`/products/${slug}`}
            className="text-sm font-semibold text-secondary hover:text-secondary-dark transition-colors duration-200"
          >
            {ctaLabel} →
          </Link>
        </div>
